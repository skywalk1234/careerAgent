...
import { ArrowDown, ArrowUp, Refresh, Star, StarFilled, Search, Switch as SwitchIcon } from '@element-plus/icons-vue'
...
const listViewMode = ref<'all' | 'favorites'>('all')
...
function applyJobQuerySnapshot(snapshot: JobQuerySnapshot) {
  Object.assign(query, {
    ...
    city: snapshot.city,
    industryTag: snapshot.industryTag,
...
function isSameJobQuerySnapshot(left: JobQuerySnapshot, right: JobQuerySnapshot) {
  return left.keyword === right.keyword
    ...
    && left.industryTag === right.industryTag
    && left.educationRequirement === right.educationRequirement
...
function normalizeJobListItem(item: JobListItem | null | undefined): JobListItem | null {
  ...
  return {
    ...
    jobCode: String(item.jobCode || '').trim(),
    companyName: String(item.companyName || '').trim() || '未知公司',
    city: String(item.city || '').trim() || '未知城市',
    district: String(item.district || '').trim() || undefined,
    industryTags: Array.isArray(item.industryTags) ? item.industryTags.map(tag => String(tag || '').trim()).filter(Boolean) : [],
    educationRequirement: String(item.educationRequirement || '').trim() || undefined,
    level: (String(item.level || '').trim() || 'middle') as JobListItem['level'],
  }
}

function normalizeSkillTextList(values: unknown) {
...
function normalizeJobDetailPayload(payload: JobDetailResult | null | undefined): JobDetailResult | null {
  ...
  return {
    ...
    industryTags: Array.isArray(payload.industryTags) ? payload.industryTags.map(tag => String(tag || '').trim()).filter(Boolean) : [],
    educationRequirement: String(payload.educationRequirement || '').trim() || undefined,
    companySize: String(payload.companySize || '').trim(),
    companyType: String(payload.companyType || '').trim(),
    level: (String(payload.level || '').trim() || 'middle') as JobDetailResult['level'],
    jobDescription: String(payload.jobDescription || '').trim() || '岗位描述待补充',
    keySkills: {
      hardSkills: normalizeSkillTextList(payload.keySkills?.hardSkills || []),
      tools: normalizeSkillTextList(payload.keySkills?.tools || []),
    },
  }
}
...
function levelLabel(level: string) {
  ...
  if (normalized === 'middle') return '中级'
  if (normalized === 'senior') return '高级'
...
function normalizeSeedMatchText(value: unknown) {
  return String(value || '')
    .toLowerCase()
    .replace(/[\s_\-\\/()（）【】\[\]·.,]+/g, '')
    .trim()
...
function resolveSeedVisualLevel(seedNodeId: string, fallbackLevel: string) {
  const fallback = normalizeJobLevel(fallbackLevel)
  const seedNorm = normalizeSeedMatchText(seedNodeId)
  const detail = selectedDetail.value

  if (detail) {
    const detailLevel = normalizeJobLevel(String(detail.level || ''))
    if (isGraphLevel(detailLevel)) {
      const categoryNorm = normalizeSeedMatchText(resolveGraphCategoryName(detail))
      const jobNorm = normalizeSeedMatchText(resolveUnifiedJobName(detail))
      const matched = !seedNorm || [categoryNorm, jobNorm].some((text) => {
        if (!text) return false
        return text === seedNorm || text.includes(seedNorm) || seedNorm.includes(text)
      })

      // When node id is generic (e.g. Java), prefer selected detail level over default middle fallback.
      if (matched || fallback === 'middle') {
        return detailLevel
      }
    }
  }

  const listLevel = normalizeJobLevel(String(
    jobs.value.find((item) => {
      const categoryNorm = normalizeSeedMatchText(item.categoryName || '')
      const jobNorm = normalizeSeedMatchText(item.jobName || '')
      return [categoryNorm, jobNorm].some((text) => {
        if (!text || !seedNorm) return false
        return text === seedNorm || text.includes(seedNorm) || seedNorm.includes(text)
      })
    })?.level || '',
  ))

  if (isGraphLevel(listLevel)) {
    return listLevel
  }
...
const listTotal = computed(() => (listViewMode.value === 'favorites' ? favoriteJobsList.value.length : total.value))
...
const companyBriefExpanded = ref(false)
const jobDescriptionExpanded = ref(false)
const keySkillsExpanded = ref(false)
const KEY_SKILL_PREVIEW_LIMIT = 6
const companyBriefText = computed(() => {
  const text = String(selectedDetail.value?.companyBrief || selectedDetail.value?.companyDescription || '').trim()
  ...
  const companyName = String(selectedDetail.value?.companyName || '该公司').trim() || '该公司'
  const sourceSite = resolveSourceSiteText(selectedDetail.value)
  return `暂无“${companyName}”的简介，可移步到岗位来源网址查看内容（站点来源：${sourceSite}）`
})
...
const canToggleCompanyBrief = computed(() => companyBriefText.value.length > 60)
const canToggleJobDescription = computed(() => !hasShortJobDescription.value && jobDescriptionText.value.length > 80)
const keySkillHardList = computed(() => normalizeSkillTextList(selectedDetail.value?.keySkills?.hardSkills || []))
const keySkillToolList = computed(() => normalizeSkillTextList(selectedDetail.value?.keySkills?.tools || []))
...
function displaySalary(row: { salaryNormalized?: string; salaryNegotiable?: boolean } | null | undefined) {
  if (row?.salaryNegotiable) return '面谈'
  const normalized = String(row?.salaryNormalized || '').trim()
  if (normalized && !/unknown/i.test(normalized)) return normalized
  return '薪资待补充'
}

function educationTagText(payload: { educationRequirement?: string } | null | undefined) {
  const text = String(payload?.educationRequirement || '').trim()
  return text || '学历待补充'
}
...
function resetDetailTextExpandedState() {
  companyBriefExpanded.value = false
...
function salaryMetaLine(detail: JobDetailResult | null) {
  if (!detail) return '-'
  if (detail.salaryNegotiable) return '薪资：可面谈'
  const normalized = String(detail.salaryNormalized || '').trim()
  return normalized && !/unknown/i.test(normalized) ? `薪资：${normalized}` : '薪资信息待补充'
}
  ...
  <section class="space-y-4">
    ...
    <el-card shadow="never">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-7">
        <el-select v-model="query.keyword" clearable filterable placeholder="岗位类别" :loading="filterLoading">
          <el-option v-for="jobName in filterOptions.jobNames || []" :key="jobName" :label="jobName" :value="jobName" />
        </el-select>
        ...
        <el-select v-model="query.industryTag" clearable filterable placeholder="行业标签" :loading="filterLoading">
          <el-option v-for="industryTag in filterOptions.industryTags" :key="industryTag" :label="industryTag" :value="industryTag" />
        </el-select>
        <el-select v-model="query.educationRequirement" clearable filterable placeholder="学历要求" :loading="filterLoading">
          <el-option
        ...
        <el-select v-model="query.level" clearable filterable placeholder="级别" :loading="filterLoading">
          <el-option v-for="level in filterOptions.levels" :key="level" :label="levelLabel(level)" :value="level" />
        </el-select>
        <el-select v-model="query.companySize" clearable filterable placeholder="公司规模" :loading="filterLoading">
          <el-option v-for="size in filterOptions.companySizes || []" :key="size" :label="size" :value="size" />
        </el-select>
        <el-select v-model="query.companyType" clearable filterable placeholder="公司类型" :loading="filterLoading">
          <el-option v-for="type in filterOptions.companyTypes || []" :key="type" :label="type" :value="type" />
        </el-select>
      </div>
          ...
          <template #header>
            <div class="flex items-center justify-between">
              ...
              <div v-if="combinedPanelView === 'list'" class="flex items-center gap-2">
                <el-checkbox v-model="latestPublishedFirst" @change="onLatestSortToggle">按最新发布时间排序</el-checkbox>
                <el-segmented
                  v-model="listViewMode"
                  size="small"
                  :options="[{ label: '全部岗位', value: 'all' }, { label: '我的收藏', value: 'favorites' }]"
                />
          ...
          <div class="combined-panel-stack">
            ...
            >
              ...
              >
                ...
                <el-table-column label="发布日期" width="125">
                  <template #default="scope">
                    ...
                    <div class="space-y-3 text-sm">
                      <div class="job-detail-hero">
                        ...
                        <div class="mt-1 text-slate-500">
                          {{ selectedDetail.companyName }} · {{ selectedDetail.city }}{{ selectedDetail.district ? `-${selectedDetail.district}` : '' }} · {{ displaySalary(selectedDetail) }}
                        </div>
                      ...
                      <div class="flex flex-wrap gap-2">
                        <el-tag effect="plain">学历要求：{{ educationTagText(selectedDetail) }}</el-tag>
                          <el-tag v-for="tag in selectedDetail.industryTags || []" :key="tag">{{ tag }}</el-tag>
                        <el-tag type="success">{{ levelLabel(selectedDetail.level) }}</el-tag>
                        <el-tag :type="publishTagType(selectedDetail)">{{ publishDateText(selectedDetail) }}</el-tag>
                        <el-tag type="warning">{{ selectedDetail.companyType || '公司类型待补充' }}</el-tag>
                        <el-tag type="success">{{ selectedDetail.companySize || '公司规模待补充' }}</el-tag>
                      </div>
                      <div class="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600">
                        <div class="mb-1 flex items-center justify-between gap-2">
                          <div class="text-sm font-medium text-slate-700">公司简介</div>
                          <el-button
                            v-if="canToggleCompanyBrief"
                            text
                            size="small"
                            class="detail-text-toggle"
                            @click="companyBriefExpanded = !companyBriefExpanded"
                          >
                            {{ companyBriefExpanded ? '缩略' : '详情' }}
                          </el-button>
                        </div>
                        <p
                          class="detail-rich-text"
                          :class="companyBriefExpanded ? 'detail-rich-text--expanded' : 'detail-rich-text--collapsed'"
                        >
                          {{ companyBriefText }}
                        </p>
                      </div>
              ...
              <template v-else>
                <div class="space-y-2">
                  ...
                  >
                    ...
                    <div class="mt-1 text-xs text-slate-500">发布日期：{{ publishDateText(item) }}</div>
                  </button>
                </div>
              </template>
            </div>
