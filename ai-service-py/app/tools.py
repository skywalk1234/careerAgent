...
async def polish_resume(
    ...
    - profile_id：要润色的简历 id（可选）。消息中可能带 `profileId:` 前缀，或用户明确指定某份简历标题/编号；
      未指定则默认润色最新一份。
    - job_id：目标岗位 id（可选）。仅当对话中能确定具体岗位（消息中带 `jobId:` 前缀或岗位编号）时才传；
      没有确定岗位时可省略，此时按用户修改要求做通用改写。
    - extra_info：本次润色的完整依据（自由文本，可空但建议充实）。请**回溯最近的对话历史**（可能不止当前这一轮，
      前文可能调用过 analyze_resume 给出诊断、用户也可能发过补充信息），把以下内容筛选、合并成一段清晰的指示传入：
        1. 前文 analyze_resume 分析结果中给出的改进建议（语言表达/结构条理/内容完整性/JD 契合度等维度的诊断要点）；
        2. 分析阶段抛出的追问以及用户随后给出的回答/补充；
        3. 用户本次明确的修改指令、想保留或想强调的经历、目标岗位名称/方向等。
      汇总原则：宁多勿漏，前文提到过的修改方向都要带进来，但不要把无关的闲聊塞入；这些内容是润色专家最主要的改写依据。
